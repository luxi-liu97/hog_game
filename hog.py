...
def roll_dice(num_rolls, dice=six_sided):
    ...
    for _ in range(num_rolls):
        ...
        if roll == 1:
            sow_sad = 1
        sum += roll
    return sum if sow_sad == 0 else sow_sad
    # END PROBLEM 1
...
def more_boar(player_score, opponent_score):
    ...
    """
    # BEGIN PROBLEM 4
    player_score_list = [int(x) for x in str(player_score)]
    opponent_score_list = [int(x) for x in str(opponent_score)]
    return min(player_score_list) < min(opponent_score_list) and max(player_score_list) > max(opponent_score_list)
    # END PROBLEM 4
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(score0, score1, last = last, running = running_high):
        if who == 0:
            score = score0
        else:
            score = score1
        ...
        if score - last > running:
            running = score - last
            print('Player', who, 'has reached a new maximum point gain.', running, 'point(s)!')
        last = score
